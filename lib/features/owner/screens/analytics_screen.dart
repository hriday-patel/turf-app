...
class _AnalyticsScreenState extends State<AnalyticsScreen> {
  ...
  void _loadAnalytics() {
    try {
      ...
      final daysBack = _selectedPeriod == '7days'
          ? 7
      ...
      showAppToast(context, '${AppStrings.analyticsLoadFailed}: $e',
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: GlassScaffoldBackground(
        child: SafeArea(
          child: Selector<BookingProvider, List<BookingModel>>(
  ...
  Widget _buildPeriodSelector() {
    final c = AppColors.of(context);
    return Tooltip(
      message: AppStrings.analyticsTooltipPeriod,
      child: DropdownButton<String>(
        ...
        items: [
          DropdownMenuItem(
              value: '7days',
              child: Text(AppStrings.analyticsPeriod7Days,
                  style: TextStyle(color: c.textPrimary))),
          DropdownMenuItem(
              value: '30days',
              child: Text(AppStrings.analyticsPeriod30Days,
                  style: TextStyle(color: c.textPrimary))),
          DropdownMenuItem(
              value: '90days',
              child: Text(AppStrings.analyticsPeriod90Days,
                  style: TextStyle(color: c.textPrimary))),
        ],
        onChanged: (val) {
          if (val != null) {
        ...
        child: Column(
          ...
          children: [
            Icon(Icons.analytics_outlined,
                size: 80, color: c.primary.withValues(alpha: 0.5)),
            const SizedBox(height: 24),
            const Text(
              AppStrings.analyticsProUnlockTitle,
              style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold),
            ),
            const SizedBox(height: 12),
            Text(
              AppStrings.analyticsProUnlockDesc,
            ...
            SizedBox(
              ...
              height: 54,
              child: Tooltip(
                message: AppStrings.analyticsTooltipUnlock,
                child: ElevatedButton(
                  ...
                  child: Text(
                    AppStrings.analyticsProUnlockCta,
                    style: TextStyle(
                        fontSize: 18,
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          // Revenue Chart
          _buildSectionTitle(AppStrings.analyticsSectionRevenue),
          _buildRevenueChart(),
          const SizedBox(height: 24),

          // Peak Hours
          _buildSectionTitle(AppStrings.analyticsSectionPeakHours),
          _buildPeakHoursChart(),
          const SizedBox(height: 24),

          // Net Utilization
          _buildSectionTitle(AppStrings.analyticsSectionNetUtilization),
          _buildNetUtilization(),
          const SizedBox(height: 24),

          // Cancellation Ratio
          _buildSectionTitle(AppStrings.analyticsSectionBookingStats),
          _buildBookingStats(),
          const SizedBox(height: 32),
        ],
      ),
  ...
  Widget _buildSectionTitle(String title) {
    ...
    return Padding(
      ...
      child: Text(
        title,
        style: TextStyle(
            fontSize: 18, fontWeight: FontWeight.bold, color: c.textPrimary),
      ),
    );
  }
      ...
      children: [
        Expanded(
            child: _buildSummaryCard(
                AppStrings.analyticsSummaryRevenue,
                PriceCalculator.formatPrice(_totalRevenue),
                c.success,
                Icons.currency_rupee)),
        const SizedBox(width: 12),
        Expanded(
            child: _buildSummaryCard(AppStrings.analyticsSummaryBookings,
                '$_totalBookings', c.primary, Icons.event_available)),
        const SizedBox(width: 12),
        Expanded(
            child: _buildSummaryCard(
                AppStrings.analyticsSummaryCancelled,
  ...
  Widget _buildRevenueChart() {
    ...
    if (spots.every((s) => s.y == 0)) {
      return _buildEmptyChart(AppStrings.analyticsEmptyRevenue);
    }
        ...
        LineChartData(
          ...
          titlesData: FlTitlesData(
            ...
            bottomTitles: AxisTitles(
              axisNameWidget: Text(AppStrings.analyticsAxisDays,
                  style: TextStyle(fontSize: 10, color: c.textSecondary)),
              axisNameSize: 18,
              sideTitles: const SideTitles(showTitles: false),
            ),
            leftTitles: AxisTitles(
              axisNameWidget: Text(AppStrings.analyticsAxisRevenue,
                  style: TextStyle(fontSize: 10, color: c.textSecondary)),
              axisNameSize: 18,
              sideTitles: const SideTitles(showTitles: false),
            ),
  ...
  Widget _buildPeakHoursChart() {
    if (_hourlyBookings.isEmpty) {
      return _buildEmptyChart(AppStrings.analyticsEmptyPeakHours);
    }
        ...
        BarChartData(
          ...
          titlesData: FlTitlesData(
            ...
            rightTitles:
                const AxisTitles(sideTitles: SideTitles(showTitles: false)),
            leftTitles: AxisTitles(
              axisNameWidget: Text(AppStrings.analyticsAxisBookings,
                  style: TextStyle(fontSize: 10, color: c.textSecondary)),
              axisNameSize: 18,
              sideTitles: const SideTitles(showTitles: false),
            ),
            bottomTitles: AxisTitles(
              axisNameWidget: Text(AppStrings.analyticsAxisHourOfDay,
                  style: TextStyle(fontSize: 10, color: c.textSecondary)),
              axisNameSize: 18,
  ...
  Widget _buildNetUtilization() {
    if (_netUtilization.isEmpty) {
      return _buildEmptyChart(AppStrings.analyticsEmptyNetUtilization);
    }
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    Text('${entry.value} bookings (${pct.toStringAsFixed(1)}%)',
      ...
      child: Row(
        children: [
          Expanded(
            child: SizedBox(
              ...
              child: total > 0
                  ...
                  : Center(
                      child: Text(AppStrings.analyticsEmptyData,
          ...
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _buildLegendItem(c.success, AppStrings.analyticsLegendConfirmed,
                  '$_totalBookings'),
              const SizedBox(height: 8),
              _buildLegendItem(c.error, AppStrings.analyticsLegendCancelled,
                  '$_cancelledBookings'),
              const SizedBox(height: 8),
              _buildLegendItem(
                  Colors.blueGrey, AppStrings.analyticsLegendTotal, '$total'),
            ],
          ),
        ],
      ),
